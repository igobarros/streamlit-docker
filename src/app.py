...
# ---------------------------- Calcula a correlação das variáveis categóri
...
def send_mail(sender_mail, password, receiver_mail, subject_matter, text_email):
    ...
    try:
        ...
        s.sendmail(str(sender_mail), str(receiver_mail),
                   msg.as_string().encode(encoding='utf-8'))
        s.quit()
...
select_datas = st.sidebar.selectbox('Escolha a extensão do arquivo', options=[
    'Selecione uma opção', 'csv', 'xlsx', 'Base tutorial - Servidor(MongoAtlas) Base(House Price - Kaggle)'
])
...
def main():
    ...
    if not isinstance(df, pd.DataFrame):
        if select_datas == 'Base tutorial - Servidor(MongoAtlas) Base(House Price - Kaggle)':
            ...
            df = pd.DataFrame.from_records(
                data=collections.find({}, projection={'_id': False}))
    ...
    if df is not None:
        ...
        exploration = pd.DataFrame({
            'columns': df.columns, 'types': df.dtypes, 'NA #': df.isna().sum(), 'NA %': (df.isna().sum() / df.shape[0]) * 100
        })
        ...
        if st.checkbox('Show raw data'):
            value = st.slider('Choose a number the lines',
            ...
            st.markdown(
                '**Algumas informações da base como (nome da coluna, tipo, números de NaNs e porcentagem de NaNs)**')
        ...
        percentual = st.slider(
            'Informe um limite de percentual de valor faltante para imputar os dados', min_value=0, max_value=100)
        num_columns_list = list(exploration[(exploration['NA %'] > percentual) & (
            exploration['types'] != 'object')]['columns'])
        ...
        if imputer == 'Imputar por -1':
            ...
            num_features.fillna(
                num_features[num_columns_list].mean(), inplace=True)

        elif imputer == 'Imputar pela mediana':
            num_features.fillna(
                num_features[num_columns_list].median(), inplace=True)

        elif imputer == 'Imputar pela moda':
            num_features.fillna(
                num_features[num_columns_list].mode(), inplace=True)
        ...
        num_fit_features = st.multiselect(
            'Selecione as features mais imporantes', options=list(num_features.columns))
        ...
        option_scaler = st.selectbox('Escolhe o método de transformação', options=(
            'Selecione uma método', 'Normalização', 'Padronização'
        ))

        if option_scaler == 'Normalização':
            for col in num_features[num_fit_features].columns:
                minmaxscaler = MinMaxScaler()
                num_features[col] = minmaxscaler.fit_transform(
                    num_features[col].values.reshape(-1, 1))
            ...
            for col in num_features[num_fit_features].columns:
                ...
                num_features[col] = standardscaler.fit_transform(
                    num_features[col].values.reshape(-1, 1))
        ...
        select_cat_corr = st.selectbox(
            'Informe a variável independente/alvo/target para calcular a correlação com as features categóricas', options=op)
        ...
        select_cat_features = st.multiselect(
            'Selecione as variáveis categóricas que serão transformadas', options=list(cat_features.columns))

        radio_cat_fit_features = st.radio('Selecione o método de transformação', options=(
            'Marque o método de transformação', 'OneHotEncoder', 'LabelEncoder'
        ))

        if radio_cat_fit_features == 'OneHotEncoder':
            for col in cat_features[select_cat_features].columns:
                onehot = OneHotEncoder(sparse=False, handle_unknown='ignore')
                cat_features[col] = onehot.fit_transform(
                    cat_features[col].values.reshape(-1, 1))

        elif radio_cat_fit_features == 'LabelEncoder':
            for col in cat_features[select_cat_features].columns:
                labelencoder = LabelEncoder()
                cat_features[col] = labelencoder.fit_transform(
                    cat_features[col].values.reshape(-1, 1))

        st.markdown('#### Base tratada - Final')
        X = pd.concat([num_features[num_fit_features],
                       cat_features[select_cat_features]], axis=1)
        ...
        select_method = st.selectbox('Selecione o método de transformação', options=(
            'Selecione uma opção', 'Raiz quadrada', 'Logarítica log', 'Logarítica log1p'
        ))
        ...
        # --------------------- DOWNLOAD DA BASE PRÉ-PROCESSADA | Suporte Técni

        select_download = st.sidebar.selectbox('Download da base | Suporte a plataforma', options=(
            'Selecione uma opção', 'Link para download', 'Dúvida sobre a plataforma'
        ))
        ...
        if select_download == 'Link para download':
            ...
            subject_matter = st.sidebar.text_input(
                label='ASSUNTO - Pressione enter e avançe para o próximo campo')
            email = st.sidebar.text_input(
                label='SEU EMAIL - Pressione enter e avançe para o próximo campo')
            text_mail = st.sidebar.text_area(
                label='MENSAGEM - Pressione Ctrl + Enter')
            ...
            if subject_matter and email and text_mail:
                ...
                send_mail(CONFIG_GMAIL['SENDER_MAIL'], CONFIG_GMAIL['PASSWORD'], CONFIG_GMAIL[
                          'RECEIVER_MAIL'], subject_matter, complete_text_mail)
        ...
        st.markdown(
            '[**Linkedin - Igo Pereira Barros**](https://www.linkedin.com/in/igo-pereira-barros-developer/)')
        st.markdown(
            '[**Linkedin - Codenation**](https://www.linkedin.com/company/code-nation)')
